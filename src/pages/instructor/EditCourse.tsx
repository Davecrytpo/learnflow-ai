...
const EditCourse = () => {
  ...
  const generateAIContent = async (lessonId: string) => {
    toast({ title: "AI Assistant", description: "Generating structured lesson content..." });
    // Simulate AI generation
    setTimeout(async () => {
      const mockContent = "<h1>Introduction to Module</h1><p>This is AI-generated content designed to facilitate rapid learning...</p>";
      await supabase.from("lessons").update({ content: mockContent }).eq("id", lessonId);
      toast({ title: "Content Generated", description: "The lesson content has been updated." });
      fetchCourseData();
    }, 2000);
  };
                ...
                <AccordionItem key={section.id} value={section.id} className="border rounded-xl bg-card overflow-hidden">
                  <AccordionTrigger className="px-4 py-3 hover:no-underline hover:bg-muted/30 transition-all">
                    <div className="flex items-center gap-3 text-left">
                      ...
                      <span className="font-bold">Section {idx + 1}: {section.title}</span>
                    </div>
                  </AccordionTrigger>
                  <AccordionContent className="p-4 pt-0 space-y-4 border-t border-border/50 bg-background/30">
                    <div className="pt-4 space-y-3">
                      {section.lessons?.map((lesson: any) => (
                        <div key={lesson.id} className="flex items-center justify-between p-3 rounded-lg border border-border bg-card group hover:border-primary/30 transition-all">
                          ...
                          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-accent" onClick={() => generateAIContent(lesson.id)}>
                      ...
                      <Button onClick={() => addLesson(section.id)} variant="ghost" className="w-full h-10 border-2 border-dashed text-muted-foreground hover:text-primary hover:border-primary/50 text-xs">
