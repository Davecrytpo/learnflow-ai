  ...
  useEffect(() => {
    const fetch = async () => {
      const { data } = await supabase
        .from("profiles")
        .select("id, user_id, display_name, institution, created_at")
      ...
      <div className="space-y-6">
        <section className="relative overflow-hidden rounded-3xl border border-border/70 bg-card/90 p-6">
          ...
          <div className="relative">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Search and manage platform users.</p>
          </div>
        </section>
              ...
              <div className="space-y-2">
                {filtered.map(u => (
                  <div key={u.id} className="rounded-xl border border-border p-3 text-sm">
                    <p className="font-medium text-foreground">{u.display_name || "N/A"}</p>
                    <p className="text-xs text-muted-foreground">{u.institution || "N/A"}</p>
                  </div>
                ))}
              </div>
            )}
