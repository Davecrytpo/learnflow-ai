...
async function seed() {
  ...
  if (adminError && adminError.message !== 'User already exists') {
    ...
    const uid = adminUser?.user?.id || (await supabase.from('profiles').select('user_id').eq('display_name', 'System Administrator').single()).data?.user_id;
  ...
  if (instError && instError.message !== 'User already exists') {
    ...
    if (!instructorId) {
        const { data } = await supabase.from('profiles').select('user_id').ilike('display_name', '%Sarah%').single();
